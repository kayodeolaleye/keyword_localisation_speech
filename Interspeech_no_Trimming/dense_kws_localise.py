...
def eval_kws_dense(full_all_utt_seg_score_dict, vocab, keyword_counts, label_dict, target_dur_dict, full_all_utt_segment_dur_dict, analyze=False):
    ...
    proposed_max_seg_dict = {}
    keyword_sigmoid_mat = [[] for i in range(len(keyword_ids))]
    for utt in utterances:

        all_utt_seg_score = full_all_utt_seg_score_dict[utt]
        max_scores = np.max(all_utt_seg_score, 0)
        max_segment = np.argmax(all_utt_seg_score, 0)
        proposed_max_seg_dict[utt] = max_segment
    ...
    for i_keyword, keyword in enumerate(keywords):       
        ...
        for utt in utt_order:
            ...
            if keyword in label_dict[utt]:
                ...
                proposed_max_seg = proposed_max_seg_dict[utt]
                proposed_dur = all_utt_segment[proposed_max_seg]
                token_max_frame = np.sum(proposed_dur[vocab[keyword]])/2
...
if __name__ == "__main__":
    ...
    full_all_utt_seg_score_dict_fn = "output/{}/full_all_utt_seg_score_dict.npz".format(args.model)
    full_all_utt_segment_dur_dict_fn = "output/{}/full_all_utt_segment_dur_dict.npz".format(args.model)
