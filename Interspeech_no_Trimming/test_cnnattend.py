...
import argparse
from utils import eval_detection_prf, extract_feature_test, get_gt_token_duration, get_logger, get_detection_metric_count, eval_localisation_accuracy, get_localisation_metric_count, eval_localisation_prf
from os import path
from config import pickle_file, device, trained_model_dir
import pickle
from utils import parse_args, spec_augment, ensure_folder
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(num_samples)):
        ...
        for tok in valid_hyp_trn:
            ...
            # plt.plot(token_attn_weight)
            # plt.xlabel("Time (frames)")
            # plt.xlim(0.0)
            # plt.ylim(0.0)
            # file_path = path.join("plots", key + "_" + tok[0] + ".pdf")
