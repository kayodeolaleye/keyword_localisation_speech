...
from utils import compute_cam, ensure_folder, extract_feature, get_gt_token_duration, get_logger, get_localisation_metric_count, eval_localisation_prf
from os import path
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(num_samples)):
        ...
        with torch.no_grad():
            full_out = model(torch.from_numpy(padded_input).unsqueeze(0).to(device))
            
        full_sigmoid_out = torch.sigmoid(full_out)

        hyp_trn = [iVOCAB[i] for i in np.where(full_sigmoid_out.squeeze(0).cpu() >= args.test_threshold)[0]]

        for gt_start_end_frame, gt_token in token_gt_duration:
            start, end = gt_start_end_frame[0], gt_start_end_frame[1]
        ...
        # GradCAM computation for a single utterance.
            cams_dict = compute_cam(grad_cam, padded_segment, iVOCAB)
        ...
        # proposed_max_durations = np.argmax(all_utt_seg_score, 0)  # size = 1000
