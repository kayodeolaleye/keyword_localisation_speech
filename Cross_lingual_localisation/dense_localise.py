...
import argparse
from utils import ensure_folder, extract_feature_train, get_gt_token_duration, get_logger, get_localisation_metric_count, eval_localisation_accuracy, eval_localisation_prf, split_frame_length
from os import path
from config import pickle_file, device, trained_model_dir, TextGrid_folder, eng_yor_word_file
import pickle
from utils import parse_args, get_eng_yor_word_dict, get_yor_eng_word_dict
...
if __name__ == "__main__":
    ...
    total = 0
    target_dur_dict = {}
    full_proposed_max_durations_dict = {}
    full_all_utt_segment_dur_dict = {}
    full_all_utt_seg_score_dict = {}
    ...
    for i in tqdm(range(num_samples)):
        ...
        for j, (start, end) in enumerate(segments_dur):
            ...
            all_utt_seg_score[j, :] = sigmoid_out
        ...
        token_gt_duration = get_gt_token_duration(key, textgrid_base_lst, yor_to_eng_word_dict, valid_gt_trn, root_path=TextGrid_folder) # ground truth start and end time for each word in utterance
        ...
        full_proposed_max_durations_dict[key] = proposed_max_durations
        full_all_utt_segment_dur_dict[key] = all_utt_segment_dur
        full_all_utt_seg_score_dict[key] = all_utt_seg_score
    ...
    np.savez_compressed(out_dir + "full_proposed_max_durations_dict.npz", **full_proposed_max_durations_dict)
    np.savez_compressed(out_dir + "full_all_utt_segment_dur_dict.npz", **full_all_utt_segment_dur_dict)
    np.savez_compressed(out_dir + "full_all_utt_seg_score_dict.npz", **full_all_utt_seg_score_dict)
