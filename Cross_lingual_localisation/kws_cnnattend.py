...
from utils import extract_feature_test, get_eng_yor_word_dict, get_gt_token_duration, get_logger, get_token_dur_dict, get_yor_eng_word_dict
from os import path
# from models.psc import PSC

from config import TextGrid_folder, pickle_file, device, trained_model_dir, keywords_fn, eng_yor_word_file
import pickle
    ...
    for i_keyword, keyword in enumerate(keywords):       
        ...
        for utt in utt_order:
            ...
            if keyword in label_dict[utt]:
                ...
                if keyword not in token_dur_dict:
                    continue
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(num_samples)):
        ...
        target_dur_dict[wave] = token_gt_duration 

        # utterances[wave] = [gt_trn, target_dur]

    print("Evaluating model's performance on keyword spotting")
