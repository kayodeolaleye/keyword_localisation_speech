...
def get_yor_eng_word_dict(filename):
    yor_eng_word_dict = {}
    with open(filename) as f:
        for line in f:
            line = line.strip().split(", ")
            if not line[1] in yor_eng_word_dict:
...
def get_gt_token_duration(key, textgrids_list, yor_to_eng_word_dict, valid_gt_trn, root_path=TextGrid_folder):
    ...
    for start_end, tok in target_dur:
        if tok == "" or tok not in yor_to_eng_word_dict.keys():
            continue
        token_eng_lst = yor_to_eng_word_dict[tok]
        if len(token_eng_lst) > 1:
            if token_eng_lst[0] in valid_token:
